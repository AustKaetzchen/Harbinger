...
def loadAndPrepareImage (image_path, max_dimension=2048):
  ...
  if image is None:
    raise FileNotFoundError(
      f"Could not read image from {image_path}. Please check the path."
    )
  ...
  if max(h, w) > max_dimension:
    ...
    print(
      f"Resized image to {new_w}x{new_h} to maintain high detail while keeping memory usage under control."
    )
  else:
